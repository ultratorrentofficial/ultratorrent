...
function build(jobs: Array<{ torrentHash: string; mediaItemId: string }>, items: any[]) {
  ...
  return { svc: new TorrentsService(registry, audit, filePath, prisma, moduleRef, { annotate: async (_e: string, ts: unknown[]) => ts.map((t) => ({ ...(t as object), parked: null })) } as never, { annotate: async (ts: unknown[]) => ts } as never), provider, mediaBulk };
}
