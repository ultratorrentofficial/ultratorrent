...
describe('TorrentsService.bulk — per-action authorization', () => {
  ...
  const svc = new TorrentsService(registry, audit, filePath, prisma, mediaBulk, { annotate: async (_e: string, ts: unknown[]) => ts.map((t) => ({ ...(t as object), parked: null })) } as never, { annotate: async (ts: unknown[]) => ts } as never);
