...
function EmailSettingsCard() {
  ...
  const [form, setForm] = useState({ host: '', port: 587, secure: false, user: '', password: '', fromName: '', fromAddress: '' });
  const [testTo, setTestTo] = useState('');
  useEffect(() => {
    if (q.data) setForm((f) => ({ ...f, host: q.data.host, port: q.data.port, secure: q.data.secure, user: q.data.user, fromName: q.data.fromName, fromAddress: q.data.fromAddress }));
  }, [q.data]);
