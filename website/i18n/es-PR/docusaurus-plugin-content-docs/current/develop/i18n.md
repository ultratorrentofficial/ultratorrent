...
└── locales/
    ├── en-US/        25 archivos de namespace
    └── es-PR/        los mismos 25, con las mismas keys
```

Los 25 namespaces:

`account`, `audit`, `auth`, `automation`, `cleanup`, `common`, `dashboard`, `engines`,
`files`, `imdb`, `indexers`, `jobs`, `media`, `mediaServerAnalytics`, `modules`, `nav`,
`prowlarr`, `rss`, `settings`, `shell`, `subtitleIntelligence`, `system`, `torrents`,
`users`, `workflows`.
...
Así que `t('feeds.titel')` es un **error de compilación**, no un string en blanco misterioso
...
// …
<h1>{t('feeds.title')}</h1>
<p>{t('feeds.ruleCount', { count: rules.length })}</p>
```
...
flowchart LR
  ...
  ESPR --> BUNDLE["Bundles JSON estáticos<br/>25 namespaces"]
  ENUS --> BUNDLE
...
1. **en-US** — `src/i18n/locales/en-US/<ns>.json`:
   ...
   ```json
   {
     "feeds": {
       ...
       "saveFailed": "Could not save the feed"
     }
   }
   ```
...
2. **es-PR** — `src/i18n/locales/es-PR/<ns>.json`, **la misma ruta de key**:
   ...
   ```json
   {
     "feeds": {
       ...
       "saveFailed": "No se pudo guardar la fuente"
     }
   }
   ```

3. **Úsalo**: `const { t } = useTranslation('rss');` → `t('feeds.created')`.
...
- **Las keys son estructurales, no oraciones.** `feeds.saveFailed`, no
  `couldNotSaveTheFeed`.
...
Añade el código a `SUPPORTED_LANGUAGES` y a `supportedLngs`, crea `locales/<code>/` con los 25
