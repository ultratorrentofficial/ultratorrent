...
Ese es todo el modelo, y es deliberadamente pequeño. "Cuando una descarga se complete, **y** su etiqueta sea `movies`, **y** su ratio esté por encima de 2.0 — muévela, y después deja de sembrarla."
...
**Acción** — qué hacer. Cada regla tiene una lista ordenada, y corren **secuencialmente**. La primera acción que lance un error **aborta las acciones restantes de esa regla** y registra la corrida como `failed`.
...
flowchart TD
    ...
    A --> A1[Acción 1]
    A1 -->|lanza error| FAIL["Abortar acciones restantes<br/>registrar 'failed'"]
    A1 -->|ok| A2[Acción 2 → …]
...
Existen dieciocho acciones, en cuatro familias.
...
`webhook` (hace POST de JSON a `params.url`).
...
El **constructor de reglas de la UI actualmente expone solo dos disparadores** (`torrent.completed`, `ratio.reached`) y **siete acciones** (`move`, `pause`, `stop`, `delete`, `delete_with_data`, `webhook`, `rename_for_media`), con campos de condición limitados a `name`, `label`, `state`, `ratio`, `size`, `progress`, `downloadRate`, `uploadRate`.

Los otros **doce disparadores** (todos los `media.*` y `rss.*`) y **once acciones** (todas las `media_*` y `rss_*`) existen en el motor y son completamente funcionales, pero solo se alcanzan **a través de la API REST** — `POST /api/automation/rules`. El catálogo completo en vivo está en `GET /api/automation/catalog`.
...
Los cinco disparadores `rss.*` corren por una ruta separada (`evaluateEvent`) que evalúa las condiciones contra un **objeto de evento plano** en vez de un torrent. Ahí solo se permiten acciones **seguras para eventos**: `webhook` y las tres acciones `rss_*`. Cualquier otro id de acción se rechaza con `Action "<type>" is not valid for an event trigger` y se registra como `failed`.
...
**2. Crea primero una regla que no pueda hacer daño.** Disparador `torrent.completed`, sin condiciones, sin ninguna acción. Guárdala, habilitada.

**3. Completa una descarga.** Abre el **Registro de ejecución** de la regla: aparece una corrida `success`, con el contexto del torrent. Acabas de probar que el disparador se dispara y que la regla coincide — que es justo lo que casi nadie verifica antes de escribir algo destructivo.
...
**5. Ahora haz algo de verdad.** Agrega una acción `move` con un destino, o una acción `rename_for_media`. El registro de ejecución guarda la corrida de cualquier modo, así que puedes ver si de verdad tuvo éxito.
...
Así que: construye la regla primero **sin ninguna acción**, mira el registro de ejecución para comprobar que el disparador y las condiciones se comportan, y solo entonces agrega la acción que elimina cosas. No hay deshacer, y una regla con cero condiciones coincide con **todo**.
:::
...
Disparador: `ratio.reached`. Condición: `ratio` `gte` `2.0`. Acción: `stop`.
...
Disparador: `rss.show.ended`. Acción: `convert_rule_to_backfill`.
...
| `Action "media_rename" is not valid for an event trigger` | Las reglas con contexto de evento (los disparadores `rss.*`) solo permiten `webhook` y las tres acciones `rss_*`. | Usa un disparador de torrent o de medios para las acciones de torrent/medios. |
...
- **Prototipa sin acciones.** Comprueba el disparador y las condiciones en el registro de ejecución antes de conectar algo destructivo. No hay simulación ni deshacer.
- **Pon las acciones inofensivas primero.** La primera acción que lance un error aborta las demás, así que el orden decide cuánto de la regla llegó a correr.
...
No. No hay endpoint de simulación, prueba ni vista previa en el módulo de automatización. Constrúyela sin acciones y mira el registro de ejecución.
...
Aborta las acciones restantes de esa regla y la corrida se registra como `failed`, con el mensaje del fallo. Las demás reglas no se ven afectadas.
...
- [ ] Crea una regla: `torrent.completed`, sin condiciones, sin acciones. Esperado: se registra una corrida `success` en la próxima finalización.
...
- [ ] Agrega dos acciones a la regla. Esperado: ambas corren, en orden.
- [ ] Haz que la segunda acción falle (un destino inválido). Esperado: la corrida registra `failed`, con el mensaje del fallo nombrando qué lanzó el error.
