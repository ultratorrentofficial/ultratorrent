...
└── locales/
    ├── en-US/        25 namespace files
    └── es-PR/        the same 25, with the same keys
```
...
So `t('feeds.titel')` is a **compile error**, not a mystery blank string at runtime.
...
flowchart LR
  ...
  ESPR --> BUNDLE["Static JSON bundles<br/>25 namespaces"]
  ENUS --> BUNDLE
...
3. **Use it**: `const { t } = useTranslation('rss');` → `t('feeds.created')`.
...
- **Keys are structural, not sentences.** `feeds.saveFailed`, not
  `couldNotSaveTheFeed`.
...
`locales/<code>/` with all 25 namespaces, and extend the fallback map. The parity test
