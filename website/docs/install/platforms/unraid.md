...
**Docker tab → Compose → Add New Stack → `ultratorrent`**, then set its **directory** to `/mnt/user/appdata/ultratorrent/ultratorrent` so it picks up the real `docker-compose.yml` and `.env`.
