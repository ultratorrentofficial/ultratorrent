...
| Bundled rTorrent restarts periodically; logs show "internal_error: priority_queue_insert(...) called on an invalid item" | A long-standing upstream bug in rTorrent `0.9.8` (present even in the newest jesec `v0.9.8-r16` build), fired during tracker-announce scheduling — **no fix exists in the 0.9.8 lineage**. It gets more frequent the more **active torrents** you run (negligible at a few, ~10/day at ~750). Each crash exits the process and the container auto-restarts, reloading the saved session (no torrents lost) but briefly pausing transfers. Mitigate by **keeping the active-torrent count modest** (remove/stop completed seeds) or, for a large library, **switch to a sturdier engine** — UltraTorrent is multi-engine and qBittorrent scales to thousands of torrents. UDP trackers and DHT are already disabled to remove the secondary crash variants. See [DOCKER.md → Optional profiles](DOCKER.md#optional-profiles). |
