...
| `media_server_analytics.*` | `view`, `view_live_activity`, `view_history`, `view_reports`, `view_users`, `export`, `manage_connections`, `manage_imports`, `run_imports`, `manage_newsletters`, `send_newsletters`, `manage_settings`, `sessions.terminate` (stop a live playback session — Concurrent Stream Control; a stronger grant than viewing, always server-enforced and audited), `stream_limits.read`/`stream_limits.manage` (view/edit concurrent-stream limits — every change audited: `stream_settings.updated`, `stream_policy.{updated,deleted,exempt_changed}`, `stream_identity.{linked,unlinked}`), `enforcement.read` (view enforcement state + history), `household.read`/`household.review`/`household.manage` (Household & Sharing — advisory account-sharing detection; view / disposition cases / configure home + networks; viewer IP evidence gated like Watch History, all admin actions audited) |
